...
class SentimentScannerPipeline:
    ...
    def collect_twitter(self, keywords: List[str] = None, max_tweets: int = 20) -> List[Dict]:
        """Fetches tweets using snscrape."""
        if keywords is None:
            keywords = ["Fed", "inflation", "interest rate"]
        ...
        for keyword in keywords[:2]:  # Limit to 2 keywords
            try:
                ...
                for tweet in sntwitter.TwitterSearchScraper(query).get_items():
                    ...
                    if likes < 100:  # Filter low engagement
                        continue
                    ...
                    self.processed_hashes.add(text_hash)
                    tweets_scraped += 1
                    logger.info(f"[Collector] Twitter: @{tweet.user.username if tweet.user else 'Unknown'} | Likes: {likes}")
            except Exception as e:
                logger.error(f"[Collector] Twitter error for '{keyword}': {e}")
    ...
    def is_relevant(text: str, engagement: Dict = None) -> bool:
        ...
        # High engagement
        if engagement.get('likes', 0) >= 1000:
            return True
